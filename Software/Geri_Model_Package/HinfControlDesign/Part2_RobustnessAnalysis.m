...
% Kphys : controller with physical inputs/outputs
% VV : vector of airspeed
...
varypzmap(P)
hold on; pzmap(P(:,:,VV==designspeed),'r'); hold off;
xlim([-60,10])
ylim([-5,60])
sgrid

varypzmap(feedback(P,C))
hold on; pzmap(feedback(P(:,:,VV==designspeed),C),'r'); hold off;
xlim([-60,10])
ylim([-5,60])
sgrid
%% FURTHER ANALYSIS 
